...
class OkrController extends Controller
{
    ...
    /**
     * Susun usulan OKR dengan AI (ChatGPT + Claude) lewat 9router.
     *
     * Menerima arahan dari form "Susun OKR dengan AI", mengirim ke 9router
     * (proxy LLM di VPS), lalu mengembalikan daftar Objective + Key Result
     * + kartu workstream untuk ditinjau user sebelum disimpan.
     *
     * Ini route sinkron — user menunggu respons. Timeout 120 detik.
     */
    public function generateAi(Request $request)
    {
        $input = $request->validate([
            'jenis_periode' => ['required', 'string', 'in:Kuartalan,Tahunan'],
            'tahun'         => ['required', 'integer', 'min:2024', 'max:2100'],
            'kuartal'       => ['required', 'string', 'in:Q1,Q2,Q3,Q4'],
            'level_okr'     => ['required', 'string', 'max:100'],
            'arahan'        => ['required', 'string', 'min:20', 'max:5000'],
            'papan_kanban'  => ['nullable', 'string', 'max:100'],
        ]);

        $orchestrator = new \App\Support\AiOrchestrator();
        $hasil = $orchestrator->susun($input);

        if (empty($hasil['ok'])) {
            return response()->json([
                'message' => $hasil['error'] ?? 'Gagal menyusun OKR dengan AI.',
                'objectives' => [],
                'logs' => $hasil['logs'] ?? [],
            ], 422);
        }
        ...
        return response()->json([
            'message'    => 'OKR berhasil disusun oleh AI.',
    ...
    /**
     * Simpan usulan OKR hasil AI ke database setelah ditinjau user.
     *
     * Menerima array objectives dari AI, membuat Objective + Key Result
     * + kartu workstream dalam satu transaksi. PIC kartu diset ke user
     * pertama yang cocok namanya, atau owner bila tidak ditemukan.
     */
    public function saveAi(Request $request)
    {
        $data = $request->validate([
            ...
            'objectives.*.description' => ['nullable', 'string', 'max:5000'],
            'objectives.*.priority' => ['nullable', 'string', 'max:50'],
            'objectives.*.omset_target' => ['nullable', 'numeric', 'min:0'],
            'objectives.*.key_results' => ['nullable', 'array', 'max:10'],
            'objectives.*.key_results.*.title' => ['required', 'string', 'max:255'],
            'objectives.*.key_results.*.source' => ['required', 'string', 'in:auto,manual,kartu'],
            'objectives.*.key_results.*.metric' => ['nullable', 'string', 'in:view,subscriber'],
            'objectives.*.key_results.*.target' => ['nullable', 'numeric', 'min:0'],
            'objectives.*.key_results.*.unit' => ['required', 'string', 'in:angka,rupiah,persen'],
            'objectives.*.key_results.*.priority' => ['nullable', 'string', 'max:50'],
            'objectives.*.key_results.*.kartu' => ['nullable', 'array', 'max:10'],
            'objectives.*.key_results.*.kartu.*.judul' => ['required', 'string', 'max:255'],
            'objectives.*.key_results.*.kartu.*.description' => ['nullable', 'string', 'max:5000'],
            'objectives.*.key_results.*.kartu.*.pic' => ['nullable', 'string', 'max:100'],
            'objectives.*.key_results.*.kartu.*.deadline' => ['nullable', 'date'],
            'objectives.*.key_results.*.kartu.*.board' => ['nullable', 'string', 'max:100'],
        ]);

        $userId = $request->user()->id;
        $priorityNames = ['Urgent', 'Penting'];
        $columnFirst = BoardColumn::where('board_key', 'todolist')->orderBy('position')->value('key') ?? 'todo';

        DB::transaction(function () use ($data, $userId, $priorityNames, $columnFirst) {
            foreach ($data['objectives'] as $aiObj) {
                $priority = $this->snapshotPriority($aiObj['priority'] ?? null, $priorityNames);

                $objective = Objective::create([
                    'year'         => (int) $data['year'],
                    'quarter'      => (int) $data['quarter'],
                    'title'        => $aiObj['title'],
                    'description'  => $aiObj['description'] ?? null,
                    'priority'     => $priority,
                    'omset_target' => $aiObj['omset_target'] ?? null,
                    'created_by'   => $userId,
                    'position'     => (int) Objective::forQuarter($data['year'], $data['quarter'])->max('position') + 1,
                ]);

                $krPos = 0;
                foreach (($aiObj['key_results'] ?? []) as $aiKr) {
                    $krPos++;
                    $krPriority = $this->snapshotPriority($aiKr['priority'] ?? null, $priorityNames);

                    $kr = KeyResult::create([
                        'objective_id' => $objective->id,
                        'title'        => $aiKr['title'],
                        'source'       => $aiKr['source'],
                        'board_key'    => ($aiKr['source'] === 'kartu') ? 'todolist' : null,
                        'metric'       => $aiKr['metric'] ?? null,
                        'target'       => $aiKr['target'] ?? 0,
                        'unit'         => $aiKr['unit'] ?? 'angka',
                        'priority'     => $krPriority,
                        'owner_id'     => $this->cariPicId($aiKr['kartu'][0]['pic'] ?? null),
                        'created_by'   => $userId,
                        'position'     => $krPos,
                    ]);

                    // Buat kartu workstream
                    foreach (($aiKr['kartu'] ?? []) as $aiCard) {
                        $boardKey = $aiCard['board'] ?? 'todolist';
                        $col = BoardColumn::where('board_key', $boardKey)->orderBy('position')->value('key') ?? $columnFirst;
                        $label = Label::where('name', $aiKr['priority'] ?? null)->first();
                        $picId = $this->cariPicId($aiCard['pic'] ?? null);
                        ...
                        Pipeline::create([
                            ...
                            'payment_status'  => 'belum',
                            'progress'        => $col,
                            'endorse'         => $aiCard['judul'],
                            'description'     => $aiCard['description'] ?? null,
                            'labels'          => $label ? [['name' => $label->name, 'group' => $label->group, 'color' => $label->color]] : [],
                            'assigned_to'     => $picId,
                            'deadline'        => $aiCard['deadline'] ?? null,
                            'key_result_id'   => $kr->id,
                            'is_kr_master'    => true,
                            'created_by'      => $userId,
                        ]);
        ...
        return back()->with('status', 'OKR AI berhasil disimpan.');
    }

    /**
     * Cari user ID dari nama PIC. Cocokkan dengan name di tabel users
     * (case-insensitive, partial match nama depan). Fallback ke owner.
     */
    private function cariPicId(?string $nama): ?int
    {
        if (empty($nama) || $nama === 'belum ditentukan') {
            return User::where('role', 'owner')->orderBy('id')->value('id');
        }

        $nama = trim($nama);
        $user = User::whereRaw('LOWER(name) LIKE ?', ['%' . strtolower($nama) . '%'])->first();

        return $user?->id ?? User::where('role', 'owner')->orderBy('id')->value('id');
    }

    /**
     * Bangun snapshot prioritas {name, color} dari nama (Urgent/Penting).
     * Warna diambil dari tabel labels; fallback ke warna default.
     */
    private function snapshotPriority(?string $nama, array $validNames): ?array
    {
        if (empty($nama) || ! in_array($nama, $validNames, true)) {
            return null;
        }

        $label = Label::where('name', $nama)->first();

        return [
            'name'  => $nama,
            'color' => $label?->color ?? ($nama === 'Urgent' ? '#dc2626' : '#2563eb'),
        ];
    }
