...
const gantiTab = (t) => {
    ...
    router.get(
        '/kpi',
        { q: props.quarter.key, tab: t === 'orang' ? 'orang' : undefined },
    ...
    <Layout :title="scope === 'semua' ? 'KPI' : 'Rapor Saya'">
            ...
            <div class="px-6 py-5 flex flex-wrap items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-brand-100 text-sm">
                        {{ quarter.label }} · {{ scope === 'semua' ? range.start + ' s/d ' + range.end : namaSaya }}
                    </p>
                </div>
                <select
                    :value="quarter.key"
        ...
        <div class="p-6">
            ...
            <div v-if="board !== null" class="flex gap-1 border-b border-brand-100 mb-5">
                <button
                    v-for="t in [
                        { key: 'board', label: 'Per Board' },
                        { key: 'orang', label: 'Per Orang' },
                    ]"
                    :key="t.key"
                    :class="[
                        'text-sm font-semibold px-4 py-2 -mb-px border-b-2 transition',
                ...
                <div class="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-6">
                    <div class="bg-white border border-brand-100 rounded-2xl p-4 shadow-sm">
                        ...
                        <p class="text-2xl font-bold text-brand-700 mt-1">
                            {{ total.done }}<span class="text-base text-slate-400">/{{ total.target > 0 ? total.target : '—' }}</span>
                        </p>
                    </div>
                    <div class="bg-white border border-brand-100 rounded-2xl p-4 shadow-sm">
                        <p class="text-xs text-slate-400">Kartu kuartal ini</p>
                        <p class="text-2xl font-bold text-slate-700 mt-1">{{ total.total }}</p>
                    </div>
                    <div class="bg-white border border-brand-100 rounded-2xl p-4 shadow-sm">
                        <p class="text-xs text-slate-400">Tepat waktu</p>
                        <p class="text-2xl font-bold text-emerald-600 mt-1">{{ total.tepat }}</p>
                    </div>
                    <div class="bg-white border border-brand-100 rounded-2xl p-4 shadow-sm">
                        <p class="text-xs text-slate-400">Terlambat</p>
                        <p class="text-2xl font-bold text-red-600 mt-1">{{ total.terlambat }}</p>
                    </div>
                    <div class="bg-white border border-brand-100 rounded-2xl p-4 shadow-sm">
                        <p class="text-xs text-slate-400">Rasio tepat waktu</p>
                        <p class="text-2xl font-bold text-brand-700 mt-1">
                            {{ total.persen_tepat === null ? '—' : total.persen_tepat + '%' }}
                        </p>
                    </div>
                </div>

                <p class="text-xs text-slate-400 mb-3">
                    Kartu masuk kuartal berdasarkan <b>deadline</b>-nya. Kartu tanpa deadline tidak dihitung di sini — target kuartalan
                    hanya bermakna untuk pekerjaan yang punya batas waktu.
                </p>
                    ...
                    <article v-for="b in board" :key="b.key" class="bg-white border border-brand-100 rounded-2xl shadow-sm p-5">
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <div class="flex items-center gap-3 shrink-0">
                                <Link
:href="urlKanban(b)"
class="text-xs font-semibold text-slate-400 hover:text-brand-700"
                                    >Buka Kanban →</Link
                                >
                                <button v-if="canManage" class="text-xs font-semibold text-brand-700 hover:underline" @click="buka(b)">
                                    Atur target
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="h-2.5 bg-slate-100 rounded-full overflow-hidden mt-2">
                            <div
                                :class="['h-full rounded-full transition-all', barColor(b.percent)]"
                                :style="{ width: barWidth(b.percent) }"
                            ></div>
                        </div>
                        ...
                        <div class="grid grid-cols-4 gap-2 mt-4 text-center">
                            <div class="rounded-xl bg-slate-50 p-2">
                                <p class="text-lg font-bold text-slate-700">{{ b.total }}</p>
                                <p class="text-[10px] text-slate-500">Kartu Q ini</p>
                            </div>
                            <div class="rounded-xl bg-emerald-50 p-2">
                                <p class="text-lg font-bold text-emerald-700">{{ b.ketepatan.tepat }}</p>
                                <p class="text-[10px] text-emerald-600">Tepat waktu</p>
                            </div>
                            <div class="rounded-xl bg-red-50 p-2">
                                <p class="text-lg font-bold text-red-700">{{ b.ketepatan.terlambat }}</p>
                                <p class="text-[10px] text-red-600">Terlambat</p>
                            </div>
                            <div class="rounded-xl bg-amber-50 p-2">
                                <p class="text-lg font-bold text-amber-700">{{ b.ketepatan.lewat }}</p>
                                <p class="text-[10px] text-amber-600">Lewat deadline</p>
                            </div>
                        </div>
                        ...
                        <p class="text-xs text-slate-400 mt-3">
                            ...
                            <b
                                :class="
                                    b.ketepatan.persen_tepat === null
                                        ? 'text-slate-400'
                                        : b.ketepatan.persen_tepat >= 80
                                          ? 'text-emerald-600'
                                          : 'text-red-600'
                                "
                            >
            ...
            <div v-else>
                ...
                <template v-if="scope === 'semua'">
                    <p class="text-xs text-slate-400 mb-3">
                        Dihitung dari kartu yang <b>ditugaskan</b> ke tiap orang, dikelompokkan lewat deadline. Yang terlambat paling banyak
                        ada di atas.
                    </p>
                    ...
                    <div v-else class="overflow-x-auto bg-white border border-brand-100 rounded-2xl shadow-sm">
                        <table class="w-full text-sm min-w-[720px]">
                            <thead>
                                <tr
                                    class="text-left text-[10px] uppercase tracking-widest text-slate-400 border-b border-slate-100 bg-slate-50"
                                >
                            ...
                            <tbody>
                                ...
                                <tr
                                    v-for="b in orang"
                                    :key="b.user_id ?? 'tanpa-pj'"
                                    :class="['border-b border-slate-50 last:border-b-0', b.user_id === null ? 'bg-slate-50/70' : '']"
                                >
                                    <td class="px-4 py-3">
                                        <span :class="b.user_id === null ? 'text-slate-400 italic' : 'font-semibold text-slate-700'">{{
                                            b.nama
                                        }}</span>
                                    </td>
                                    ...
                                    <td class="px-4 py-3 text-right tabular-nums text-slate-600">
                                        {{ b.rata_telat === null ? '—' : b.rata_telat + ' hari' }}
                                    </td>
                ...
                <template v-else>
                    ...
                    <div v-else class="space-y-4">
                        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
                            <div class="bg-white border border-brand-100 rounded-2xl p-4 shadow-sm">
                                <p class="text-xs text-slate-400">Kartu saya</p>
                                <p class="text-2xl font-bold text-brand-700 mt-1">{{ raporSaya.total }}</p>
                            </div>
                            <div class="bg-white border border-brand-100 rounded-2xl p-4 shadow-sm">
                                <p class="text-xs text-slate-400">Selesai</p>
                                <p class="text-2xl font-bold text-slate-700 mt-1">{{ raporSaya.selesai }}</p>
                            </div>
                            <div class="bg-white border border-brand-100 rounded-2xl p-4 shadow-sm">
                                <p class="text-xs text-slate-400">Tepat waktu</p>
                                <p class="text-2xl font-bold text-emerald-600 mt-1">{{ raporSaya.tepat }}</p>
                            </div>
                            <div class="bg-white border border-brand-100 rounded-2xl p-4 shadow-sm">
                                <p class="text-xs text-slate-400">Terlambat</p>
                                <p class="text-2xl font-bold text-red-600 mt-1">{{ raporSaya.terlambat }}</p>
                            </div>
                        </div>
                        ...
                        <div class="bg-white border border-brand-100 rounded-2xl shadow-sm p-5 space-y-2">
                            ...
                            <div class="h-2.5 bg-slate-100 rounded-full overflow-hidden">
                                <div
                                    :class="['h-full rounded-full transition-all', barColor(raporSaya.persen_tepat)]"
                                    :style="{ width: barWidth(raporSaya.persen_tepat) }"
                                ></div>
                            </div>
                            <p class="text-xs text-slate-400">
                                ...
                                <span v-if="raporSaya.lewat > 0">
                                    · {{ raporSaya.lewat }} kartu sudah lewat deadline dan belum selesai.</span
                                >
                            </p>
                        </div>
                    </div>
            ...
            <form class="mt-4 space-y-3" @submit.prevent="simpan">
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-500 mb-1"
                        >Catatan <span class="font-normal text-slate-400">(opsional)</span></label
                    >
                    <textarea
                        v-model="form.note"
                        rows="2"
                ...
                <div class="flex justify-end gap-2 pt-2">
                    <button type="button" class="px-4 py-2 text-sm font-semibold text-slate-500 hover:text-slate-700" @click="modal = null">
                        Batal
                    </button>
                    <button
                        type="submit"
                        :disabled="form.processing"
                        class="px-4 py-2 text-sm font-semibold text-white bg-brand-600 hover:bg-brand-700 rounded-xl disabled:opacity-50"
                    >
                        Simpan
                    </button>
                </div>
