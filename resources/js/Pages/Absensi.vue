...
const hapus = (a) => {
    if (confirm('Hapus pengajuan ini?')) router.delete(`/absensi/${a.id}`, { preserveScroll: true });
};
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        ...
                        <tr v-for="a in absences" :key="a.id" class="border-b border-brand-50 last:border-0 align-top">
                            ...
                            <td class="px-4 py-3">
                                <span :class="['text-xs font-semibold px-2 py-0.5 rounded', typeClass(a.type)]">{{
                                    types[a.type] || a.type
                                }}</span>
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <a
                                    ...
                                    target="_blank"
                                    class="text-brand-600 hover:text-brand-800 text-xs font-medium underline"
                                    >Lihat</a
                                >
                                <span v-else class="text-slate-300 text-xs">—</span>
                            </td>
                            <td class="px-4 py-3">
                                <span :class="['text-xs font-semibold px-2 py-0.5 rounded', statusClass(a.status)]">{{
                                ...
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <template v-if="canManage && a.status === 'menunggu'">
                                        <button
                                            ...
                                            @click="setStatus(a, 'disetujui')"
                                        >
                                            Setujui
                                        </button>
                                        <button
                                            class="text-xs font-semibold text-red-600 hover:text-red-800"
                                            @click="setStatus(a, 'ditolak')"
                                        >
                                            Tolak
                                        </button>
                                    </template>
        ...
        <ModalWrap v-if="open" width="max-w-md" @close="open = false">
            ...
            <form class="space-y-4 text-sm" @submit.prevent="submit">
                <div>
                    ...
                    <select
                        v-model="form.type"
                        class="w-full border border-slate-200 rounded-xl px-3 py-2.5 bg-white focus:ring-2 focus:ring-brand-400 outline-none"
                    >
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        <label class="block font-medium text-slate-600 mb-1"
                            >Sampai <span class="text-slate-400 font-normal">(opsional)</span></label
                        >
                        <input
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        <label
                            for="absensi-file"
                            class="cursor-pointer inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 text-xs font-semibold"
                        >
                            Pilih file
                        </label>
                        <input id="absensi-file" type="file" accept=".jpg,.jpeg,.png,.pdf" class="hidden" @change="pickFile" />
