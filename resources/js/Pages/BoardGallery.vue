...
// ---- Hapus board ----
const deleteBoard = (b) => {
    if (confirm(`Hapus board "${b.name}"? (hanya bila kosong)`)) router.delete('/boards/' + b.key);
};
    ...
    <Layout title="Kanban">
        <div class="p-6">
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <button
                    v-if="canManage"
                    class="inline-flex items-center gap-1.5 bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition"
                    @click="openCreate"
                >
            ...
            <div v-if="boards.length === 0" class="bg-white border border-dashed border-brand-200 rounded-2xl p-12 text-center">
                <svg class="w-12 h-12 mx-auto text-brand-300 mb-3" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M4 5h4v14H4zM10 5h4v9h-4zM16 5h4v6h-4z" />
                </svg>
                ...
                <button
                    v-if="canManage"
                    class="inline-flex items-center gap-1.5 bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold px-4 py-2 rounded-xl transition"
                    @click="openCreate"
                >
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
            ...
            <div v-for="(list, section) in grouped" :key="section" class="mb-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    <div v-for="b in list" :key="b.key" class="relative group">
                        ...
                        <Link
                            :href="`/pipelines/kanban?category=${b.key}`"
                            class="block bg-white border border-brand-100 rounded-2xl shadow-sm hover:shadow-md hover:border-brand-300 transition p-5 h-full"
                        >
                            <div class="flex items-start justify-between mb-8">
                                ...
                                <span
                                    v-if="b.super_admin_only"
                                    title="Board khusus"
                                    class="inline-flex items-center gap-1 text-[10px] font-semibold text-amber-700 bg-amber-50 border border-amber-200 rounded-full px-2 py-0.5"
                                >
                        ...
                        <div v-if="canManage" class="absolute top-3 right-3">
                            <button
                                class="w-7 h-7 flex items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 opacity-0 group-hover:opacity-100 transition"
                                @click.stop.prevent="menuOpen = menuOpen === b.key ? null : b.key"
                            >
                            ...
                            <div
                                v-if="menuOpen === b.key"
                                class="absolute right-0 top-8 z-20 w-36 bg-white border border-brand-100 rounded-xl shadow-lg py-1 text-sm"
                            >
                                <button
        ...
        <ModalWrap v-if="canManage && createOpen" width="max-w-sm" @close="createOpen = false">
            ...
            <form class="space-y-3 text-sm" @submit.prevent="submitCreate">
                ...
                <label class="block font-medium text-slate-600"
                    >Section / Grup (opsional)
                    <input
                        v-model="createForm.section"
                        placeholder="mis. Billy Expense"
                        class="mt-1 w-full border border-slate-200 rounded-xl px-3 py-2 focus:ring-2 focus:ring-brand-400 outline-none"
                    />
                ...
                <div class="flex justify-end gap-2 pt-1">
                    ...
                    <button
                        ...
                        class="px-4 py-2 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-semibold disabled:opacity-60"
                    >
                        Simpan
                    </button>
                </div>
        ...
        <ModalWrap v-if="canManage && editOpen" width="max-w-sm" @close="editOpen = false">
            ...
            <form class="space-y-3 text-sm" @submit.prevent="submitEdit">
                <label class="block font-medium text-slate-600"
                    >Nama board
                    <input
