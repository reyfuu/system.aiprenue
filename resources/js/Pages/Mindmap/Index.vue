...
// Hapus
const remove = (m) => {
    if (confirm(`Hapus mindmap "${m.title}"?`)) router.delete('/mindmaps/' + m.id);
};
</script>
        ...
        <div class="p-6">
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <button
                    ...
                    class="inline-flex items-center gap-1.5 bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition disabled:opacity-60"
                    @click="pilihOpen = true"
                >
            ...
            <div v-if="mindmaps.length === 0" class="bg-white border border-dashed border-brand-200 rounded-2xl p-12 text-center">
                <svg class="w-12 h-12 mx-auto text-brand-300 mb-3" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                    <path
                        ...
                        d="M8 6a2 2 0 11-4 0 2 2 0 014 0zm12 0a2 2 0 11-4 0 2 2 0 014 0zm0 12a2 2 0 11-4 0 2 2 0 014 0zM8 6h4a2 2 0 012 2m0 8a2 2 0 00-2-2H8"
                    />
                </svg>
                ...
                <button
                    v-if="canManage"
                    :disabled="createForm.processing"
                    class="inline-flex items-center gap-1.5 bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold px-4 py-2 rounded-xl transition disabled:opacity-60"
                    @click="pilihOpen = true"
                >
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
            ...
            <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                <div v-for="m in mindmaps" :key="m.id" class="relative group">
                    ...
                    <button
                        v-if="canManage"
                        title="Hapus"
                        class="absolute top-3 right-3 w-7 h-7 flex items-center justify-center rounded-lg text-slate-400 hover:bg-red-50 hover:text-red-600 opacity-0 group-hover:opacity-100 transition"
                        @click.stop.prevent="remove(m)"
                    >
            ...
            <ModalWrap v-if="pilihOpen" width="max-w-2xl" @close="pilihOpen = false">
                <div class="flex items-start justify-between mb-4">
                    ...
                    <button class="text-slate-400 hover:text-slate-600 text-xl leading-none" @click="pilihOpen = false">&times;</button>
                </div>
