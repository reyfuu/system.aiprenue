...
const fmtDate = (value) => (value ? new Date(value).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' }) : '—');
            ...
            <div class="bg-white border border-brand-100 shadow-sm rounded-2xl p-4 mb-5 flex flex-wrap items-end gap-3">
                <label class="text-sm font-medium text-slate-600">
                    ...
                    <input
                        v-model="week"
                        type="week"
                        class="mt-1 block border border-slate-200 rounded-xl px-3 py-2 focus:ring-2 focus:ring-brand-400 outline-none"
                        @change="applyWeek"
                    />
                </label>
                <button
                    v-if="week"
                    class="px-4 py-2 text-sm text-slate-600 hover:text-brand-700"
                    @click="
                        week = '';
                        applyWeek();
                    "
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-[2600px] w-full text-xs text-left">
                        <thead class="bg-brand-50 text-slate-700 uppercase">
                            <tr>
                                <th class="px-3 py-3">Comp</th>
                                <th class="px-3 py-3">Jenis Postingan</th>
                                <th class="px-3 py-3">Kategori</th>
                                <th class="px-3 py-3">Referensi</th>
                                <th class="px-3 py-3">Inti Pesan</th>
                                <th class="px-3 py-3">Hook &amp; Material</th>
                                <th class="px-3 py-3">Brief Original</th>
                                <th class="px-3 py-3">Opsi Brief/Fiks</th>
                                <th class="px-3 py-3">Script Remake</th>
                                <th class="px-3 py-3">Editor</th>
                                <th class="px-3 py-3">Progress</th>
                                <th class="px-3 py-3">Tanggal Upload</th>
                                <th class="px-3 py-3">Link Hasil Editing</th>
                                <th class="px-3 py-3">Link B Roll</th>
                                <th class="px-3 py-3">Caption</th>
                                <th class="px-3 py-3">Link AI + Kata Kunci</th>
                                <th v-if="canManage" class="px-3 py-3 sticky right-0 bg-brand-50">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            <tr v-if="!contents.data.length">
                                <td :colspan="canManage ? 17 : 16" class="px-4 py-12 text-center text-slate-400">
                                    Belum ada content pada filter ini.
                                </td>
                            </tr>
                            <tr v-for="content in contents.data" :key="content.id" class="align-top hover:bg-brand-50/40">
                                ...
                                <td class="px-3 py-3 w-48">
                                    <a
                                        v-if="isUrl(content.referensi)"
                                        :href="content.referensi"
                                        target="_blank"
                                        rel="noopener"
                                        class="text-blue-600 hover:underline"
                                        >Buka referensi</a
                                    ><span v-else class="line-clamp-4 whitespace-pre-line">{{ content.referensi || '—' }}</span>
                                </td>
                                <td class="px-3 py-3 w-52">
                                    <p class="line-clamp-5 whitespace-pre-line">{{ content.inti_pesan || '—' }}</p>
                                </td>
                                <td class="px-3 py-3 w-60">
                                    <p class="line-clamp-5 whitespace-pre-line">{{ content.hook_material || '—' }}</p>
                                </td>
                                <td class="px-3 py-3 w-72">
                                    <p class="line-clamp-6 whitespace-pre-line">{{ content.brief_original || '—' }}</p>
                                </td>
                                <td class="px-3 py-3 w-64">
                                    <p class="line-clamp-6 whitespace-pre-line">{{ content.opsi_brief || '—' }}</p>
                                </td>
                                <td class="px-3 py-3 w-72">
                                    <p class="line-clamp-6 whitespace-pre-line">{{ content.script_remake || '—' }}</p>
                                </td>
                                <td class="px-3 py-3 w-32">{{ content.editor || '—' }}</td>
                                <td class="px-3 py-3 w-28">
                                    <span
                                        :class="['inline-flex rounded-full px-2.5 py-1 font-semibold', progressClass(content.progress)]"
                                        >{{ progressOptions[content.progress] || content.progress }}</span
                                    >
                                </td>
                                <td class="px-3 py-3 w-32">{{ fmtDate(content.tanggal_upload) }}</td>
                                <td v-for="field in ['link_hasil_editing', 'link_b_roll']" :key="field" class="px-3 py-3 w-44">
                                    <a
                                        v-if="isUrl(content[field])"
                                        :href="content[field]"
                                        target="_blank"
                                        rel="noopener"
                                        class="text-blue-600 hover:underline"
                                        >Buka link</a
                                    ><span v-else>{{ content[field] || '—' }}</span>
                                </td>
                                <td class="px-3 py-3 w-64">
                                    <p class="line-clamp-6 whitespace-pre-line">{{ content.caption || '—' }}</p>
                                </td>
                                <td class="px-3 py-3 w-48">
                                    <a
                                        v-if="isUrl(content.link_ai_kata_kunci)"
                                        :href="content.link_ai_kata_kunci"
                                        target="_blank"
                                        rel="noopener"
                                        class="text-blue-600 hover:underline"
                                        >Buka link AI</a
                                    ><span v-else class="whitespace-pre-line">{{ content.link_ai_kata_kunci || '—' }}</span>
                                </td>
                                <td v-if="canManage" class="px-3 py-3 sticky right-0 bg-white whitespace-nowrap">
                                    <button class="text-brand-700 font-semibold mr-3" @click="openEdit(content)">Edit</button>
                                    <button class="text-red-600 font-semibold" @click="destroyContent(content)">Hapus</button>
                                </td>
                            </tr>
                        </tbody>
                    </table>
            ...
            <div v-if="contents.last_page > 1" class="mt-5 flex flex-wrap gap-1">
                <Link
                    ...
                    :class="[
                        ...
                        link.active ? 'bg-brand-700 text-white border-brand-700' : 'bg-white text-slate-600 border-slate-200',
        ...
        <ModalWrap v-if="open" width="max-w-6xl" align="items-start" @close="open = false">
            <form @submit.prevent="submit">
                <div class="flex items-center justify-between mb-5">
                    ...
                    <button type="button" class="text-slate-400 text-xl" @click="open = false">×</button>
                </div>
                <div class="grid md:grid-cols-2 gap-4 max-h-[70vh] overflow-y-auto pr-2">
                    <label
                        v-for="field in ['comp', 'jenis_postingan', 'kategori', 'editor']"
                        :key="field"
                    ...
                    >
                        ...
                        <input
                            v-model="form[field]"
                            class="mt-1 w-full border border-slate-200 rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-brand-400"
                        />
                    ...
                    <label class="text-sm font-medium text-slate-600"
                        >Progress<select
                            ...
                            class="mt-1 w-full border border-slate-200 rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-brand-400"
                        >
                            <option v-for="(label, key) in progressOptions" :key="key" :value="key">{{ label }}</option>
                        </select></label
                    >
                    <label class="text-sm font-medium text-slate-600"
                        >Tanggal Upload<input
                            v-model="form.tanggal_upload"
                            type="date"
                            class="mt-1 w-full border border-slate-200 rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-brand-400"
                        /><span v-if="form.errors.tanggal_upload" class="text-xs text-red-600">{{
                            form.errors.tanggal_upload
                        }}</span></label
                    >
                    <label
                        v-for="field in ['referensi', 'link_hasil_editing', 'link_b_roll', 'link_ai_kata_kunci']"
                        :key="field"
                        class="text-sm font-medium text-slate-600"
                    >
                        {{
                            {
                                referensi: 'Referensi',
                                link_hasil_editing: 'Link Hasil Editing',
                                link_b_roll: 'Link B Roll',
                                link_ai_kata_kunci: 'Link AI + Kata Kunci',
                            }[field]
                        }}
                        <textarea
                            v-model="form[field]"
                    ...
                    <label
                        v-for="field in ['inti_pesan', 'hook_material', 'brief_original', 'opsi_brief', 'script_remake', 'caption']"
                        :key="field"
                        class="text-sm font-medium text-slate-600 md:col-span-2"
                    >
                        {{
                            {
                                inti_pesan: 'Inti Pesan',
                                hook_material: 'Hook & Material',
                                brief_original: 'Brief Original',
                                opsi_brief: 'Opsi Brief/Fiks',
                                script_remake: 'Script Remake',
                                caption: 'Caption',
                            }[field]
                        }}
                        <textarea
                ...
                <div class="mt-5 flex justify-end gap-2">
                    <button type="button" class="px-4 py-2 text-slate-600" @click="open = false">Batal</button>
                    <button
                        :disabled="form.processing"
                        class="px-5 py-2 rounded-xl bg-brand-700 text-white font-semibold disabled:opacity-50"
                    >
