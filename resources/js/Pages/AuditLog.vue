...
<template>
  <Layout title="Audit Log">
    ...
    <div class="flex flex-wrap gap-2 mb-5">
      <select v-model="form.user_id" @change="filter" class="border border-gray-300 rounded px-2 py-1 text-sm">
        <option :value="null">Semua User</option>
        <option v-for="u in users" :key="u.id" :value="u.id">{{ u.name }} ({{ u.role }})</option>
      </select>
      <select v-model="form.action" @change="filter" class="border border-gray-300 rounded px-2 py-1 text-sm">
        <option :value="null">Semua Aksi</option>
        <option v-for="a in actions" :key="a" :value="a">{{ a }}</option>
      </select>
      <select v-model="form.model_type" @change="filter" class="border border-gray-300 rounded px-2 py-1 text-sm">
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-3 py-2">
              ...
              >{{ showDetail === log.id ? 'Sembunyi' : 'Lihat' }}</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="logs.last_page > 1" class="mt-4 flex justify-center">
      <div class="flex gap-1 text-sm">
        <button
          ...
          :class="page === logs.current_page ? 'bg-indigo-600 text-white' : 'bg-white border border-gray-300 text-gray-600 hover:bg-gray-50'"
    ...
    <ModalWrap :show="showDetail !== null" @close="showDetail = null" maxWidth="lg">
      <template #title>Detail Perubahan</template>
      <template v-if="showDetail" v-for="log in logs.data.filter(l => l.id === showDetail)" :key="log.id">
          ...
          <div>
            ...
            <div v-if="log.new_values" class="space-y-0.5">
              <div v-for="(val, key) in log.new_values" :key="key" class="flex gap-1">
                <span class="text-gray-400 font-mono text-xs w-32 truncate">{{ key }}</span>
                <span
                  :class="{ 'text-green-700': !log.old_values || log.old_values[key] !== val }"
                  class="break-all"
